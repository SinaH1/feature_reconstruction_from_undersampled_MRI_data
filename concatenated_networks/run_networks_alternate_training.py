...
from adjusted_VarNet.models_learned_kernels import *
...
def main():
	...
	args['device'] = device
	if args['decreasing_loss_weight']:
		args['loss_weight_varnet_unet'] = 0.6

	args['image_size'] = [192,192]
	args['num_coils'] = 4
	args['loss'] = BinaryDice_and_BCE_loss({},{})
	if args['net_type'] == 'learned_kernels':
		args['num_input_channels'] = 4
	elif args['net_type'] == '1_kernel':
	...
	param_group_unet_cells = []
	for i in range(args['num_stages']):
		param_group_varnet_cells.append({'params': concat_net.varnet.cell_list[i+1].parameters(), 'lr':args['lr']})
	param_group_unet_cells.append({'params': concat_net.varnet.cell_list[0].parameters(), 'lr':args['lr']})
	param_group_unet_cells.append({'params': concat_net.segm_unet.parameters(), 'lr':args['lr']})

	optimizer_varnet = torch.optim.Adam(param_group_varnet_cells,lr=args['lr'])
	optimizer_unet = torch.optim.Adam(param_group_unet_cells,lr=args['lr'])
	...
	if args['mode'] == 'train':
		...
		for epoch in range(args['epoch']):
			if args['decreasing_loss_weight'] and (epoch%int(args['epoch']/5)==0):
				args['loss_weight_varnet_unet'] -= 0.1
				print('loss_weight_varnet_unet:', args['loss_weight_varnet_unet'])
			concat_net.train()
			...
			for batch_idx, batch in enumerate(data_dqdm_loop):
				...
				l_varnet = args['loss_weight_varnet_unet'] * args['weight_varnet'] * F.mse_loss(output_varnet, concat_net.get_varnet_reference(batch['reference']).to(output_varnet.device))
				l_unet = (1-args['loss_weight_varnet_unet']) * args['loss'](output, batch['seg'])
				l = l_varnet + l_unet
				if batch_idx%2==0:
					optimizer_varnet.zero_grad()
					l_varnet.backward()
					optimizer_varnet.step()
				else:
			...
			if args['check_validation_per_epoch']:
				...
				for batch_idx, batch in enumerate(data_dqdm_loop):
					...
					batch = {k: v.to(device=device, non_blocking=True) for k, v in batch.items()}
					output, output_varnet = concat_net(batch)
					l_varnet_val = args['loss_weight_varnet_unet'] * args['weight_varnet'] * F.mse_loss(output_varnet, concat_net.get_varnet_reference(batch['reference']).to(output_varnet.device))
					l_unet_val = (1-args['loss_weight_varnet_unet']) * args['loss'](output, batch['seg'])
					l_val = l_varnet_val + l_unet_val
				...
				if l_val_losses_per_epoch[-1] <= best_val_loss:
					if args['net_type'] in ['original','learned_feat','learned_kernels','1_kernel','first_derivative']:
		...
		if args['net_type'] in ['original','learned_feat','1_kernel','learned_kernels','first_derivative']:
			varnet.load_state_dict(torch.load(str(save_dir / 'varnet_best.h5')))
			for i in range(args['num_input_channels']):
				print(varnet.cell_list[0].filter_kernels[i])
			mse_loss = []
			ssim = []
			for batch_idx, batch in enumerate(data_dqdm_loop):
				...
				#mse_loss_torch.append(out['test_loss_torch'])
