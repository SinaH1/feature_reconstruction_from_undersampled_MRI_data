...
from adjusted_VarNet.fft_utils import complex_2_numpy
import numpy as np
import imageio
import torch
import matplotlib
...
def save_recon(under, recon, reference, index, save_dir, error_scale=1,do_save=True, network_type='original', rescale=False):
    ...
    recon: torch tensor (NxHxWx2) or tupel of torch tensors (NxHxWx2)
    ...
    network_type: str
        choose between 'original', 'feature_edges' for different outputs
    ...
    elif network_type=='dx':
        recon_np = complex_2_numpy(recon[0].detach().cpu())
    ...
    for i in range(batch_size):
        ...
        if network_type == 'original':
            img_to_save = np.concatenate((under_mag,recon_mag,reference_mag,diff),axis=1)
            if rescale:
...
def save_recon_per_cascade(save_dir, recon, error_scale=1,do_save=True, network_type='original'):
    """ Save the reconstruction result and compare with reference
    ...
    recon_mag /= np.max(recon_mag)

    img_to_save = 255 * recon_mag
    if do_save:
        save_name = save_dir
        imageio.imwrite(str(save_name),img_to_save.astype(np.uint8))
