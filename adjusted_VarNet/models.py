...
class RBFActivationFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, w, mu, sigma):
        ...
        ----------
        ctx: 
        input: torch tensor (B x features_out x H+pad x W+pad)
            input tensor
        ...
        sigma: torch tensor (1)
            std of the RBF
        Returns: (B x features_out x H+pad x W+pad)
        ----------
...
class VariationalNetwork(nn.Module):   
    ...
    def forward_for_evaluation_each_cascade(self,inputs):
        outputs = [inputs]
        for i in range(len(self.cell_list)):
            outputs.append(self.cell_list[i](outputs[-1]))
        return outputs
    ...
    def test_step(self, batch, batch_idx, save=True, save_recon_np=False):
        ...
        recon_img_mag = torch_abs(recon_img)
        if save_recon_np:
            save_dir = Path(self.options['save_dir']+f'/numpy_recon')
            save_dir.mkdir(parents=True,exist_ok=True)
            for i in range(recon_img_mag.shape[0]):
                np.save(self.options['save_dir']+f'/numpy_recon/recon_{1* batch_idx + i}.npy', recon_img.detach().cpu().numpy())

        if self.options['loss_type'] == 'complex':
            loss = F.mse_loss(recon_img,ref_img)
            ssim = []
            for i in range(recon_img.shape[0]):
                ssim.append(structural_similarity(ref_img[i,:,:,0].cpu().detach().numpy(), recon_img[i,:,:,0].cpu().detach().numpy()))
        elif self.options['loss_type'] == 'magnitude':
            recon_img_mag = torch_abs(recon_img)
            ref_img_mag = torch_abs(ref_img)    
    ...
    def eval_images_per_cascade(self,batch, batch_idx, save=True):
        outputs = [batch]
        u_t = []
        for i in range(len(self.cell_list)):
            outputs.append(self.cell_list[i](outputs[-1]))
            u_t.append(outputs[-1]['u_t'])
        if save:
            save_dir = Path(self.options['save_dir']+f'/images_per_cascade')
            save_dir.mkdir(parents=True,exist_ok=True)
            for i in range(u_t[0].shape[0]):
                for j in range(len(self.cell_list)-1):
                    save_recon_per_cascade(self.options['save_dir']+f'/images_per_cascade/recon_{1* batch_idx + i}_{j}.png', u_t[j][0,:,:,:])
