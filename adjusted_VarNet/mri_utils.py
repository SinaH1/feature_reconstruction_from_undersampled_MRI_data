...
def fft2c(img):
    """ Centered fft2 """
    #return np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(img), norm='ortho')) / np.sqrt(img.shape[-2]*img.shape[-1])
    return np.fft.fft2(img, norm='ortho') #/ np.sqrt(img.shape[-2]*img.shape[-1])

def ifft2c(img):
    """ Centered ifft2 """
    #return np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(img), norm='ortho')) * np.sqrt(img.shape[-2]*img.shape[-1])
    return np.fft.ifft2(img, norm='ortho') #/ np.sqrt(img.shape[-2]*img.shape[-1])
...
def imsave(img, filepath, normalize=False):
    """ Save an image. """
    path = os.path.dirname(filepath) or '.'
    if not os.path.exists(path):
        os.makedirs(path)
